...
void Capture_helper(FILE *captureData, unsigned char *buffer, int size)
{
    ...
    if (etherHeader->h_proto == ETH_P_IP)
    {
        /* all 프로토콜 선택시*/
...
void Icmp_header_capture(FILE *captureData, struct ethhdr *etherHeader, struct iphdr *ipHeader, unsigned char *Buffer, int Size)
{
    ...
    if (!strcmp(ipOption, "*") || !strcmp(inet_ntoa(source.sin_addr), ipOption) || !strcmp(inet_ntoa(dest.sin_addr), ipOption))
    {
        ...
        time_t t = time(NULL);
        struct tm tm = *localtime(&t);
        fprintf(stdout, "\n%d:%d:%06d IPv%d %s > ", tm.tm_hour, tm.tm_min, tm.tm_sec, (unsigned int)ipHeader->version, inet_ntoa(source.sin_addr));
...
void Icmp_header_fprint(FILE *captureData, unsigned char *Buffer, struct ethhdr *etherHeader, struct iphdr *ipHeader, struct icmphdr *icmpHeader, struct sockaddr_in source, struct sockaddr_in dest, int Size)
{
    ...
    fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(captureData, "                  Type             |   %d", (unsigned int)(icmpHeader->type));
    fprintf(captureData, "           --------------------------------------------------------\n");
...
bool start_helper(char *str)
{
    ...
    /*출력 option*/
    option = strtok(NULL, "\n");
    if (strcmp(option, "a") && strcmp(option, "s") && strcmp(option, "x"))
    {
        fprintf(stderr, "잘못된 Option 입니다.\n");
        return false;
    }
    strcpy(printOption, option);
