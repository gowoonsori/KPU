...
void Tcp_header_fprint(FILE *, unsigned char *, struct ethhdr *, struct iphdr *, struct tcphdr *, struct sockaddr_in, struct sockaddr_in, int);   //tcp 헤더 정보 fprint
void Udp_header_capture(FILE *captureFile, struct ethhdr *, struct iphdr *, unsigned char *, int);                                                //udp 헤더 정보 capture
void Udp_header_fprint(FILE *, unsigned char *, struct ethhdr *, struct iphdr *, struct udphdr *, struct sockaddr_in, struct sockaddr_in, int);   //udp 헤더 정보 fprint
void Icmp_header_capture(FILE *captureFile, struct ethhdr *, struct iphdr *, unsigned char *, int);                                               //icmp 헤더 정보 capture
void Icmp_header_fprint(FILE *, unsigned char *, struct ethhdr *, struct iphdr *, struct icmphdr *, struct sockaddr_in, struct sockaddr_in, int); //icmp 헤더 정보 fprint
...
void Ethrenet_header_fprint(FILE *captureData, struct ethhdr *etherHeader)
{
    ...
    fprintf(captureData, "               Src MAC Addr          |      [%02x:%02x:%02x:%02x:%02x:%02x]\n", // 6 byte for src
            etherHeader->h_source[0], etherHeader->h_source[1], etherHeader->h_source[2], etherHeader->h_source[3], etherHeader->h_source[4], etherHeader->h_source[5]);
    fprintf(captureData, "               Dst MAC Addr          |      [%02x:%02x:%02x:%02x:%02x:%02x]\n", // 6 byte for dest
            etherHeader->h_dest[0], etherHeader->h_dest[1], etherHeader->h_dest[2], etherHeader->h_dest[3], etherHeader->h_dest[4], etherHeader->h_dest[5]);
    fprintf(captureData, "           --------------------------------------------------------\n\n");
}
...
void Tcp_header_capture(FILE *captureData, struct ethhdr *etherHeader, struct iphdr *ipHeader, unsigned char *Buffer, int Size)
{
    ...
    { //filter port번호 검사
        if (!strcmp(portOption, "*") || (atoi(portOption) == (int)ntohs(tcpHeader->source)) || (atoi(portOption) == (int)ntohs(tcpHeader->dest)))
        {
            /*현재 시간 get*/
            struct timeb itb;
            ftime(&itb);
            ...
            if ((unsigned int)tcpHeader->urg == 1)
                fprintf(stdout, "U.");
            if ((unsigned int)tcpHeader->ack == 1)
                fprintf(stdout, "A.");
            if ((unsigned int)tcpHeader->psh == 1)
                fprintf(stdout, "P.");
            if ((unsigned int)tcpHeader->rst == 1)
                fprintf(stdout, "R.");
            if ((unsigned int)tcpHeader->syn == 1)
                fprintf(stdout, "S.");
            if ((unsigned int)tcpHeader->fin == 1)
                fprintf(stdout, "F.");
            fprintf(stdout, "], seq %u, ack %u, win %d, length %d", ntohl(tcpHeader->seq), ntohl(tcpHeader->ack_seq), ntohs(tcpHeader->window), Size);
            ...
            if (!strcmp(printOption, "a"))
            {
                ...
                Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof tcpHeader, A, (Size - sizeof tcpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
            }
            /*hex 출력*/
            else if (!strcmp(printOption, "x"))
            {
                ...
                fprintf(stdout, "\n\033[101mpayload\t\033[0m");
                Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof tcpHeader, X, (Size - sizeof tcpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
            }
            /*file 출력*/
...
void Udp_header_capture(FILE *captureData, struct ethhdr *etherHeader, struct iphdr *ipHeader, unsigned char *Buffer, int Size)
{
    ...
    if (!strcmp(ipOption, "*") || !strcmp(inet_ntoa(source.sin_addr), ipOption) || !strcmp(inet_ntoa(dest.sin_addr), ipOption))
    { //port 번호 filter 검사
        if (!strcmp(portOption, "*") || (atoi(portOption) == (int)ntohs(udpHeader->source)) || (atoi(portOption) == (int)ntohs(udpHeader->dest)))
        {
            /*현재 시간 get*/
            struct timeb itb;
            ftime(&itb);
            ...
            if (!strcmp(printOption, "a"))
            {
                ...
                fprintf(stdout, "\n\033[101mpayload\t\033[0m");
                Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof udpHeader, A, (Size - sizeof udpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
            }
            /*hex 출력*/
            else if (!strcmp(printOption, "x"))
            {
                ...
                fprintf(stdout, "\n\033[101mpayload\t\033[0m");
                Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof udpHeader, X, (Size - sizeof udpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
            }
            /*file 출력*/
...
void Udp_header_fprint(FILE *captureData, unsigned char *Buffer, struct ethhdr *etherHeader, struct iphdr *ipHeader, struct udphdr *udpHeader, struct sockaddr_in source, struct sockaddr_in dest, int Size)
{
    fprintf(captureData, "\n############################## UDP Packet #####################################\n");
    ...
    Change_hex_to_ascii(captureData, Buffer + (ipHeader->ihl * 4) + sizeof udpHeader, F, (Size - sizeof udpHeader - (ipHeader->ihl * 4) - ETH_HLEN));
    fprintf(captureData, "\n===============================================================================\n");
}
...
void Icmp_header_capture(FILE *captureData, struct ethhdr *etherHeader, struct iphdr *ipHeader, unsigned char *Buffer, int Size)
{
    ...
    if (!strcmp(ipOption, "*") || !strcmp(inet_ntoa(source.sin_addr), ipOption) || !strcmp(inet_ntoa(dest.sin_addr), ipOption))
    {
        ...
        ftime(&itb);
        struct tm *tm = localtime(&itb.time);
        fprintf(stdout, "\n%02d:%02d:%02d:%03d IPv%d %s > ", tm->tm_hour, tm->tm_min, tm->tm_sec, itb.millitm, (unsigned int)ipHeader->version, inet_ntoa(source.sin_addr));
        fprintf(stdout, "%s = ICMP [Type : %d/Code : %d] TTL=%d ", inet_ntoa(dest.sin_addr), (unsigned int)icmpHeader->type, (unsigned int)icmpHeader->code, (unsigned int)ipHeader->ttl);
        fprintf(stdout, "length %d", Size);
        ...
        if (!strcmp(printOption, "a"))
        {
            ...
            fprintf(stdout, "\n\033[101mpayload\t\033[0m");
            Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof icmpHeader, A, (Size - sizeof icmpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
        }
        /*hex 출력*/
        else if (!strcmp(printOption, "x"))
        {
            ...
            Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof icmpHeader, X, (Size - sizeof icmpHeader - (ipHeader->ihl * 4) - ETH_HLEN)); //payload
        }
        /*file 출력*/
        Icmp_header_fprint(captureData, Buffer, etherHeader, ipHeader, icmpHeader, source, dest, Size);
    }
}
void Icmp_header_fprint(FILE *captureData, unsigned char *Buffer, struct ethhdr *etherHeader, struct iphdr *ipHeader, struct icmphdr *icmpHeader, struct sockaddr_in source, struct sockaddr_in dest, int Size)
{
    fprintf(captureData, "\n############################## ICMP Packet ####################################\n");
    Ethrenet_header_fprint(captureData, etherHeader);      //ethernet 정보 print
    Ip_header_fprint(captureData, ipHeader, source, dest); //ip 정보 print
    ...
    Change_hex_to_ascii(captureData, Buffer + ETH_HLEN + (ipHeader->ihl * 4) + sizeof icmpHeader, F, (Size - sizeof icmpHeader - (ipHeader->ihl * 4) - ETH_HLEN));
    fprintf(captureData, "\n===============================================================================\n");
}
...
void Change_hex_to_ascii(FILE *captureData, unsigned char *data, int op, int Size)
{
    ...
    else if (op == F)
    {
        ...
        {
            if (i != 0 && i % 16 == 0)
            {                                    //16개 데이터 출력 했다면, ascii코드 출력후 개행후 이어서 출력
            ...
            if (i % 16 == 0)
                fprintf(captureData, "\t"); //가시성을 위해 처음 오는 data는 tab
...
void StartMenuBoard()
{
    ...
    fprintf(stdout, "\n**************************** Start Rule ***************************\n\n");
    fprintf(stdout, "                입력 순서 :  \033[100mprotocol\033[0m \033[100mport\033[0m \033[100mip\033[0m \033[100moption\033[0m \n");
    fprintf(stdout, "\n*******************************************************************\n\n");
}
...
bool start_helper(char *str)
{
    ...
    if (strcmp(option, "*") && strcmp(option, "tcp") && strcmp(option, "udp") && strcmp(option, "icmp") && strcmp(option, "http") && strcmp(option, "dns"))
    {
